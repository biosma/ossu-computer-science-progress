...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for key in freq_dict1:
        if key not in freq_dict2:
            freq[key] = freq_dict1[key]
        elif key in freq_dict2:
            freq[key] = abs(freq_dict1[key] - freq_dict2[key])
    for key in freq_dict2:
        if key not in freq:
            freq[key] = freq_dict2[key]
    if sum(freq_dict1.values()) + sum(freq_dict2.values()) == 0:
        return 1.0
    return round(1 - (sum(freq.values()) / (sum(freq_dict1.values()) + sum(freq_dict2.values()))), 2)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for word in freq_dict1:
        if word in freq_dict2:
            freq_helper[word] = freq_dict1[word] + freq_dict2[word]
        else:
            freq_helper[word] = freq_dict1[word]
    for word in freq_dict2:
        if word not in freq_helper:
            freq_helper[word] = freq_dict2[word]
    for word in freq_helper:
        if freq_helper[word] == max(freq_helper.values()):
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    words = text.lower().split()  # asume que las palabras están separadas por espacios o saltos de línea
    frequency_dict = get_frequencies(words)        
    tf_dict = {}
    for word in frequency_dict:
        tf_dict[word] = frequency_dict[word] / sum(frequency_dict.values())
    return tf_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for file_path in file_paths:
        text = load_file(file_path)
        words = list(set(words).union(text.split()))
        dict_helper[file_path] = get_letter_frequencies(text.split())

    for word in words:
        for file_path in file_paths:
            if word in dict_helper[file_path]:
                if word in documents_word_dict:
                    documents_word_dict[word] += 1
                else:
                    documents_word_dict[word] = 1
                    
    for word in documents_word_dict:
        idf_dict[word] = math.log10(len(file_paths) / documents_word_dict[word])
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    * TF-IDF(i) = TF(i) * IDF(i)
    """
    sorted_list = []
    text = get_tf(tf_file_path)
    dict_helper = get_idf(idf_file_paths)
    for word in text:
        sorted_list.append((word, text[word] * dict_helper[word]))
