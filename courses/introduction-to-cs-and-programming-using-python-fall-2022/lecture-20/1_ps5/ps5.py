...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        # converts each pixel to a tuple of ints
        new_pixels_list.append(tuple(int(c) for c in matrix_multiply(matrix, pixel)))
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    if type(pixel) == tuple:
        return tuple(c % (2 ** num_end_bits) for c in pixel)
...
def reveal_bw_image(filename):
    ...
    end_bits_list = []
    for pixel in pixels_list:
        end_bits_list.append(extract_end_bits(1, pixel) * 255)
    img_result = pix_to_img(end_bits_list, img.size, 'L')
    img_result.save(filename + "_revealed.png")
    return img_result
...
def reveal_color_image(filename):
    ...
    end_bits_list = []
    for pixel in pixels_list:
        end_bits_list.append(tuple(int(channel * 255/7) for channel in extract_end_bits(3, pixel)))
    img_result = pix_to_img(end_bits_list, img.size, 'RGB')
    img_result.save(filename + "_revealed.png")
    return img_result
...
def main():
    ...
    im3 = pix_to_img(red_filtered_pixels,(width,height), 'RGB')
    im3.save('red_filtered.png')
    # im3.show()
    ...
    # im2.show()
    # draw_kerb('red_filtered.png', 'kerb')
    # draw_kerb('hidden1.bmp_revealed.png', 'kerb')
    # draw_kerb('hidden2.bmp_revealed.png', 'kerb')
